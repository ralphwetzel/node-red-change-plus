...
let evaluate = async function(_RED, _node, func, msg, done) {
    ...
    let function_script = function(_script) {
    ...
    try {
        // Try to treat script as an Expression
        script = vm.createScript(function_script(`return ${func}`), createVMOpt(node, ""));
    } catch (err) {
        // Treat it as a function script
        try {
            script = vm.createScript(function_script(func), createVMOpt(node, ""));
        } catch (err2) {
            // Syntax Error?
            done(err.toString());
            return;
